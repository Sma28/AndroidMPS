...
public class ImageUtils {
     ...
     *
     */
    public static int[][][] RGBArray(Bitmap img) {
        int[][][] rgb = null;
        ...
        if (height > 0 && width > 0) {
            ...
            for (int row = 0; row < height; row++) {
                for (int column = 0; column < width; column++) {
                    rgb[row][column] = intRGB(img.getPixel(column, row));
                }
            }
        }
     ...
     */
    public static Bitmap RGBImg(int[][][] raw) {
        Bitmap img = null;
        ...
        if (height > 0 && width > 0 || raw[0][0].length == 3) {
            ...
            for (int row = 0; row < height; row++) {
                for (int column = 0; column < width; column++) {
                    img.setPixel(column, row, (raw[row][column][0] << 16) | (raw[row][column][1] << 8) | (raw[row][column][2]));
                }
            }
        }
     ...
     */
    public static int[][] GSArray(Bitmap img) {
        int[][] gs = null;
        int height = img.getHeight();
        int width = img.getWidth();
     ...
     */
    public static Bitmap GSImg(int[][] raw) {
        Bitmap img = null;
        int height = raw.length;
        int width = raw[0].length;
        ...
        if (height > 0 && width > 0) {
            ...
            for (int i = 0; i < height; i++) {
                for (int j = 0; j < width; j++) {
                    img.setPixel(j, i, (raw[i][j] << 16) | (raw[i][j] << 8) | (raw[i][j]));
                }
            }
        }
     ...
     * @deprecated
     */
    public static double[][][] HSVArray(Bitmap img) {
        if (img == null) {
        ...
        for (int r = 0; r < height; r++) {
            for (int c = 0; c < width; c++) {
                int[] rgb = intRGB(img.getPixel(c, r));
     ...
     * @deprecated
     */
    public static double[][][] HSIArray(Bitmap img) {
        if (img == null) {
            throw new IllegalArgumentException("ERROR: Source image is null!");
        }
     ...
     * @deprecated
     */
    public static double[][][] TSLArray(Bitmap img) {
        if (img == null) {
            throw new IllegalArgumentException("ERROR: Source image is null!");
        }
     ...
     * @deprecated
     */
    public static Bitmap HSVImg(double[][][] raw) {
        int height = raw.length;
        int width = raw[0].length;
        ...
        for (int r = 0; r < height; r++) {
            for (int c = 0; c < width; c++) {
                ...
                img.setPixel(c, r, (rgb[0] << 16) | (rgb[1] << 8) | (rgb[2]));
            }
        }
     ...
     * @deprecated
     */
    public static Bitmap HSIImg(double[][][] raw) {
        int height = raw.length;
        int width = raw[0].length;
     ...
     * @deprecated
     */
    public static double[][][] YCbCrArray(Bitmap img) {
        if (img == null) {
            throw new IllegalArgumentException("ERROR: Source image is null!");
        }
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                int[] rgb = intRGB(img.getPixel(j, i));
                //Y
